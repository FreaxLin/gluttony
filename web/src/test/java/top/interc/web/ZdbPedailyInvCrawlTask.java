...
public class ZdbPedailyInvCrawlTask {
    ...
    @Test
    public void test(){
        String url = "https://zdb.pedaily.cn/inv/p%s";
        int i = 1;
        while (true){
            try {
                Thread.sleep(2000);
                ...
                for (Element e : invList){
                    ...
                    if (e.select("div.info").get(0).getElementsByTag("span").size() == 1){
                        if (e.select("div.info").get(0).getElementsByTag("span").get(0).text().equals("非公开")){
                            record.setInvTurn(e.select("div.info").get(0).getElementsByTag("span").get(0).text());
                        }
                    }else if (e.select("div.info").get(0).getElementsByTag("span").size() > 1){
                        if (!e.select("div.info").get(0).getElementsByTag("span").get(0).text().contains("轮")){
                            record.setInvMoney(e.select("div.info").get(0).getElementsByTag("span").get(0).text()
                                    + e.select("div.info").get(0).getElementsByTag("span").get(1).text());
                        }else{
                            record.setInvTurn(e.select("div.info").get(0).getElementsByTag("span").get(0).text());
                            if (e.select("div.info").get(0).getElementsByTag("span").size() > 2){
                                record.setInvMoney(e.select("div.info").get(0).getElementsByTag("span").get(1).text()
                                        + e.select("div.info").get(0).getElementsByTag("span").get(2).text());
                            }
                        }
                    }
